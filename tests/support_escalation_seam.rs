...
//!
//! Tenancy (ADR-0029): backbone-project is NOT tenant-agnostic — its Project rows carry the legacy
//! owner-company key. Support sources that SIBLING domain key from the ambient org scope and fails
//! closed without one, so the probe binds a company-anchored org request scope around the escalations
//! (the composition-time posture a composing service installs for every request).
...
async fn seseam1_issue_escalates_to_real_project() {
    ...
    let project = RealProject {
        svc:
            backbone_project::application::service::project_write_service::ProjectWriteService::new(
                pool.clone(),
            ),
        pool: pool.clone(),
    };
    ...
    let issue = svc
        .raise_issue(
            NewIssue {
                ...
                subject: "Onsite repair needed".into(),
                description: None,
                priority: "high".into(),
                sla_id: Some(sla),
            },
    ...
    // The sibling port's owner-company key comes from the ambient org scope — bind it explicitly.
    let (project_id, again) = with_company_scope(&pool, company, async {
        let a = svc
    ...
    assert_eq!(
        cust,
        Some(customer),
        "delivery project is for the ticket's customer"
    );
    assert_eq!(ptype, "external");
    assert!(
        name.contains(&issue.to_string()),
        "project names the originating issue"
    );
    ...
    // Idempotent: a second escalate opened no second project.
