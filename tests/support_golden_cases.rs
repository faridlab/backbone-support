...
//!
//! Tenancy (ADR-0029): these cases run undecorated — no tenant scaffolding. Under a composing
//! service's row fence the same verbs are fenced by the scope the composer binds; the module
//! suite proves only the tenant-agnostic domain rules.
...
async fn sla_high_4h(svc: &SupportWriteService) -> Uuid {
...
fn high_issue(sla: Uuid) -> NewIssue {
    NewIssue {
...
async fn sgc1_raise_snapshots_deadlines() {
    ...
    let sla = sla_high_4h(&svc).await;
    let issue = svc
        .raise_issue(high_issue(sla), dt("2026-07-07T09:00:00Z"))
        .await
        .unwrap();
    ...
    assert_eq!(
        rb,
        dt("2026-07-07T10:00:00Z"),
        "response_by = opened + 60 min"
    );
    assert_eq!(
        resb,
        dt("2026-07-07T13:00:00Z"),
        "resolution_by = opened + 240 min"
    );
}
...
async fn sgc2_resolve_within_deadline_fulfilled() {
    ...
    let issue = svc
        ...
        .await
        .unwrap();
    svc.record_first_response(issue, dt("2026-07-07T09:30:00Z"))
        .await
        .unwrap();

    let fulfilled = svc
        .resolve_issue(issue, dt("2026-07-07T12:00:00Z"), &sink)
        .await
        .unwrap();
    assert!(fulfilled, "resolved 12:00 < 13:00 deadline");
    let agr: String =
        sqlx::query_scalar("SELECT agreement_status::text FROM support.issues WHERE id=$1")
            .bind(issue)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn sgc3_resolve_past_deadline_failed() {
    ...
    let fulfilled = svc
        .resolve_issue(issue, dt("2026-07-07T14:00:00Z"), &sink)
        .await
        .unwrap();
...
async fn sgc4_pause_extends_the_clock() {
    ...
    svc.pause_sla(issue, dt("2026-07-07T10:00:00Z"))
        .await
        .unwrap();
    svc.resume_sla(issue, dt("2026-07-07T12:00:00Z"))
        .await
        .unwrap();

    let (resb, paused): (DateTime<Utc>, i32) =
        sqlx::query_as("SELECT resolution_by, total_paused_mins FROM support.issues WHERE id=$1")
            .bind(issue)
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(
        ...
        "resolution_by pushed out by the 2h hold"
    );
...
async fn sgc5_validation() {
    ...
    let no_priority = svc
        ...
        .await;
    assert!(
        matches!(no_priority, Err(SupportError::Invalid(_))),
        "SLA needs a priority target"
    );
    ...
    let bad_times = svc
        .create_sla(NewSla {
            ...
            priorities: vec![NewSlaPriority {
                priority: "low".into(),
                response_time_mins: 240,
                resolution_time_mins: 60,
            }],
        })
        .await;
    assert!(
        matches!(bad_times, Err(SupportError::Invalid(_))),
        "resolution must be >= response"
    );
    ...
    let no_subject = svc
        ...
        .await;
    assert!(
        matches!(no_subject, Err(SupportError::Invalid(_))),
        "issue needs a subject"
    );
    ...
    let no_target = svc
        ...
        .await;
    assert!(
        matches!(no_target, Err(SupportError::Invalid(_))),
        "no SLA target for this priority"
    );
}
