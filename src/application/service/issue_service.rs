//! Application service for Issue entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `issue_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateIssueDto, UpdateIssueDto};
use crate::domain::entity::Issue;
use crate::infrastructure::persistence::IssueRepository;

/// Application service for Issue entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `issue_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct IssueServiceCustom {
///     inner: Arc<IssueService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type IssueService = GenericCrudService<
    Issue,
    CreateIssueDto,
    UpdateIssueDto,
    IssueRepository,
>;

// <<< CUSTOM
// END CUSTOM
